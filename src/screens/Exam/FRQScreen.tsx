                ...
                <View className="bg-white pt-12 pb-6 px-6 shadow-sm">
                    ...
                    <View className="mb-4">
                        <View className="flex-row justify-between items-center mb-2">
                            ...
                            <TouchableOpacity
                                onPress={toggleTimer}
                                className={`flex-row items-center px-3 py-1 rounded-lg ${isTimerActive ? 'bg-teal-100' : 'bg-gray-100'
                                    }`}
                            >
                                <Clock size={16} color={isTimerActive ? "#3CBCB2" : "#9CA3AF"} />
                                <Text className={`ml-1 text-sm ${isTimerActive ? 'text-teal-700' : 'text-gray-500'}`}>
                ...
                >
                    <View className="bg-white rounded-2xl p-6 my-6 shadow-sm border border-gray-100">
                        ...
                        <View className="flex-row justify-between items-center mt-4">
                            <Text className="text-sm text-gray-600">
                                Từ: {currentAnswer.split(' ').filter(word => word.length > 0).length}
                            </Text>
                            <Text className="text-sm text-gray-600">
                                ...
                                <View key={question.id} className="flex-row items-center justify-between">
                                    ...
                                    {answers[question.id] && (
                                        <Text className="text-xs text-blue-600">
                                            {answers[question.id].split(' ').filter(word => word.length > 0).length} từ
                                        </Text>
                                    )}
                                </View>
                            ))}
                    ...
                    <View className="bg-green-50 rounded-xl p-4 mb-8">
                        <Text className="text-sm font-medium text-green-800 mb-2">💡 Mẹo làm bài:</Text>
                        <Text className="text-sm text-green-700">
                            • Đọc kỹ câu hỏi và yêu cầu{'\n'}
                            • Cấu trúc câu trả lời rõ ràng{'\n'}
                            • Sử dụng từ vựng phong phú{'\n'}
                            • Kiểm tra lỗi chính tả trước khi nộp
                        </Text>
                    </View>
                </ScrollView>
            </View>
