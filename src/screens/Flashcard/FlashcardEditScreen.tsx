...
type NavigationProp = NativeStackNavigationProp<FlashcardStackParamList, 'FlashcardEdit'>;
type RouteProps = RouteProp<FlashcardStackParamList, 'FlashcardEdit'>;

interface CardInput extends CreateFlashcardRequest {
    id: string;
}

const FlashcardEditScreen: React.FC = () => {
    ...
    useEffect(() => {
        if (currentFlashcardSet && !isInitialized) {
            ...
            setCards(
                currentFlashcardSet.flashcards.map((fc) => ({
                    id: fc.id,
                    term: fc.term,
    ...
    const handleCardChange = useCallback(
        (cardId: string, field: keyof CreateFlashcardRequest, value: string) => {
            setCards((prev) =>
    ...
    const handleSubmit = async () => {
        ...
        const result = await updateFlashcardSet(flashcardSetId, {
            title: title.trim(),
        ...
        Alert.alert(
            ...
            [
                { text: 'Cancel', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        setIsDeleting(true);
                        const success = await deleteFlashcardSet(flashcardSetId);
                        setIsDeleting(false);
                        if (success) {
                            navigation.navigate('MyFlashcards');
                        } else {
                    ...
                    <View style={styles.switchRow}>
                        <View>
                            ...
                            <Text style={styles.switchDescription}>
                                {isVisible
                                    ? 'Public - Anyone can see this set'
                ...
                >
                    {isDeleting ? (
                        ...
                        <>
                            ...
                            <Text style={styles.deleteButtonText}>Delete Flashcard Set</Text>
                        </>
                    )}
                </TouchableOpacity>
